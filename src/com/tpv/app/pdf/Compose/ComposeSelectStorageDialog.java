...
public class ComposeSelectStorageDialog extends Fragment {
    ...
    class BtnClickListener implements View.OnClickListener {
        public void onClick(View v) {
            ...
            if(mBtn_focusid == R.id.dialog_btn_usb) {
                Utils.getFragmanetManager().beginTransaction()
                        .replace(R.id.main_container, new ComposeSelectUsbDialog(), "ComposeSelectUsbDialog")
                        .addToBackStack(null)
                        .commit();
            }
            else {
                boolean showToast = false;
                if(mBtn_focusid == R.id.dialog_btn_sdcard) {
                    if(Utils.getSDCardStoragePath().contains("null/")) {
                        Utils.Toast(mCtx, getString(R.string.sdcard_not_exits));
                        showToast = true;
                    }
                }
                if(!showToast) {
